...
class Bookmark
  ...
  def initialize(*args)
    super
    if File.exist?('bookmark.yaml') 
      @bookmarks = YAML.load_file('bookmark.yaml')
    else
      @bookmarks = {}
    end
  end
  ...
  def execute(m, query)
    if query.split.length >1
      ...
      if v[1] == "bookmark"
        # @bookmarks[v[2]] ||= []
        # @bookmarks[v[2]] << v[3]
        if @bookmarks.has_key?(v[2])
          m.reply "Bookmark already exists"
        else
          @bookmarks.store(v[2], v[3])
          m.reply "bookmarked " + v[2]  
          update_bookmarks
        end
      elsif v[1] == "unmark"  
  ...
  def update_bookmarks
    synchronize(:update) do
      File.open('bookmark.yaml', 'w') do |fh|
        YAML.dump(@bookmarks, fh)
      end
    end
  end
